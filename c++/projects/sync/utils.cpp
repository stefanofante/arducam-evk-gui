...
const std::map<int, int> cvt_code_map{
    {0, cv::COLOR_BayerRG2BGR},
    {1, cv::COLOR_BayerGR2BGR},
    {2, cv::COLOR_BayerGB2BGR},
    {3, cv::COLOR_BayerBG2BGR},
    {4, cv::COLOR_GRAY2BGR},
};
...
cv::Mat from_image(ArducamImageFrame image)
{
    ...
    // Auto-stretch contrast so a dim/low-exposure signal is still visible.
    // Useful for high bit-depth monochrome sensors (e.g. Mira220 12-bit) where
    // raw values are bunched at the low end after the bit_width shift.
    if (!color_frame.empty())
    {
        double minV = 0.0, maxV = 0.0;
        if (color_frame.channels() == 1)
        {
            cv::minMaxLoc(color_frame, &minV, &maxV);
        }
        else
        {
            cv::Mat gray;
        ...
        if ((dbg_counter++ % 30) == 0)
        {
            printf("[debug] frame min=%g max=%g\n", minV, maxV);
        }
        if (maxV - minV >= 1.0)
        {
            cv::normalize(color_frame, color_frame, 0, 255, cv::NORM_MINMAX);
        }
    }
